...
function renderWelcomeMessage(){
    ...
    document.getElementById('welcome_message').innerHTML = `<img src="/static/assets/icons/shake_hands.png" alt=""><span class="font_white_color"> Welcome </span>${user.fullname}!`
}

function renderBoardList(){
    let htmlText = "";
    currentBoards.forEach(board => {
        htmlText += `<li><a class="link" href="/board/?id=${board.id}/">${board.title}</a></li>`
    });
...
function getSingleTaskTemplate(task) {
    ...
    return `        <tr onclick="redirectToBoardWTask(${task.id})">
                        ...
                        <td class="d_flex_cc_gs task_count" zero="${task.comments_count==0}"> 
                            ...
                            <img src="../../static/assets/icons/comment_bubble_filled.svg" alt="" srcset="">
                            <img src="../../static/assets/icons/comment_bubble_empty.svg" alt="" srcset="">
                        </td>
                        <td>
                            ${assigneeTemplate}
                        </td>
