...
function getDetailTaskPersonTemplate(member){
    if (member) {
        ...
        return `<img src="/static/assets/icons/face_icon.svg" alt="">
                            <p>unassigned</p>`
    }
}

function getSingleCommmentTemplate(comment) {
    let delete_btn = comment.author == getAuthFullname() ? `<img src="/static/assets/icons/delete.svg" class="delete_btn" alt="" onclick="deleteComment(${comment.id})">` : ""
...
function getTaskCreateMemberListEntrieTemplate(type, currentBoard) {
    let listHtml = `<li onclick="unsetMemberAs('${type}'); toggleDropdown(this, event)">
                        <img src="/static/assets/icons/face_icon.svg" alt="">
                        <p>unassigned</p>
                    </li>`
...
function getBoardCardTemplate(task) {
    ...
    return `            <li class="column_card" onclick="openTaskDetailDialog(${task.id})">
                            <header class="column_card_header">
                                ...
                                <div class="d_flex_sc_gm">
                                    <img src="/static/assets/icons/${task.priority}_prio_colored.svg" alt="">
                                    ${assignee_html}
                                </div>
                            </header>
...
function getBoardCardMoveBtnTemplate(task) {
    ...
    if (currentStatusIndex > 0) {
        moveBtns += `<button onclick="modifyTaskStatus(${task.id}, '${statii[currentStatusIndex-1]}')">${statii[currentStatusIndex-1]}<img class="rotate_half" src="/static/assets/icons/arrow_forward.svg" alt="" srcset=""></button>`
    }
    if (currentStatusIndex < statii.length - 1) {
        moveBtns += `<button onclick="modifyTaskStatus(${task.id}, '${statii[currentStatusIndex+1]}')">${statii[currentStatusIndex+1]} <img src="/static/assets/icons/arrow_forward.svg" alt="" srcset=""></button>`
    }

    return `<div move-open="false" class="move_btn" onclick="toggleMoveOpen(this); stopProp(event)">
        <img src="/static/assets/icons/swap_horiz.svg" alt="">
